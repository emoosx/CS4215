...
let compile (e:sPL_expr) : sVML_prog_sym   =
  let rec helper (ce:c_env) (e:sPL_expr) : sVML_prog_sym * sVML_prog_sym  =
    match e with
      ...
      | Cond (e1,e2,e3) ->
         begin
         let l_else = labels # fresh_id and l_end = labels # fresh_id in
         let (s1,p1) = helper ce e1 in
         let (s2,p2) = helper ce e2 in
         let (s3,p3) = helper ce e3 in
         (s1@([JOF l_else]@s2@[(GOTO l_end); (LABEL l_else)]@s3@[(LABEL l_end)]), p1@p2@p3)
        end
      | Appln (f,_,args) ->
         begin
            let s,p = List.split(List.map(fun x -> helper ce x) args) in
            let sx = List.flatten(List.rev s) and px = List.flatten p in
      ...
      | RecFunc (t,f,vs,body) -> 
         begin
            ...
            let (s1,p1) = helper new_ce body in
            let fvs_n = List.map
                        (fun v -> match (Environ.get_val new_ce v) with
                                    | Some i -> (v, i)
                                    | _ -> (v, -1)) (f::fvs) in
...
let main =
  ...
  if String.length !Globals.file == 0 then print_endline Globals.usage else 
    let _ = print_endline "Loading sPL program .." in
    let (s,p) = Genparse.parse_file !Globals.file in
    ...
    match v with
      ...
      | Some t ->
            ...
            let fn = extract_filename !Globals.file in
