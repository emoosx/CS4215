...
let _ = Genparse.parse option_flag 
...
let rec type_infer (env:env_type) (e:sPL_expr) : sPL_type option * sPL_expr =
  let type_infer (env:env_type) (e:sPL_expr) : sPL_type option * sPL_expr =
    let pr1 = string_of_sPL in
    let pr_out = pr_pair (pr_option string_of_sPL_type) string_of_sPL in
    Debug.no_2 "type_infer" pr_none pr1 pr_out type_infer env e in
  match e with
      ...
      | Appln (f,t,args) ->
            begin
              match t with
                | Some t1 ->
                      begin
                        let () = Debug.tinfo_pprint "inside the Appln\n" Globals.no_pos in
...
let trans_exp (e:sPL_expr) : C.sPL_expr  =
  let pr1 = string_of_sPL in
  let pr2 = C.string_of_sPL in
  Debug.no_1 "trans_exp" pr1 pr2 trans_exp e 
